            ...
            <div class="x_content">
                <div class="row clearfix">
                    ...
                    <div class="form-group col-md-3">
                        ...
                        <div class="col-md-9">
                            <select class="form-control" name="tipoVar">
                                @foreach($tipoVars as $t)
                                <option value="{{$t}}" {{($t == $busqueda['tipoVar'])? 'selected="selected"':''}}
                                >{{$t}}</option>
                                @endforeach								
                            </select>
                        </div>
...
<!--span>
    {{$url}}&origen={{$busqueda['origen']}}&fecha={{$busqueda['fecha']}}&tipoVar={{$ssrs['tipoVar']}}&banca={{$ssrs['banca']}}&zonal={{$ssrs['zonal']}}&segmento={{$ssrs['segmento']}}&ejecutivo={{$ssrs['ejecutivo']}}
</span-->
<iframe id="frameSSRS" 
        src="{{$url}}&origen={{$busqueda['origen']}}&fecha={{$busqueda['fecha']}}&tipoVar={{$ssrs['tipoVar']}}&banca={{$ssrs['banca']}}&zonal={{$ssrs['zonal']}}&segmento={{$ssrs['segmento']}}&ejecutivo={{$ssrs['ejecutivo']}}" 
...
<script>
    ...
    function cboJefaturaChange(banca, zonal, jefatura, ejecutivo) {
        ...
        var cboEjecutivo = $('#cboEjecutivo');
        var sizeJefatura = $('#cboJefatura option').size();
        ...
        $.ajax({
            ...
            success: function (json) {
                ...
                if (ejecutivo){
                    ...
                    if (sizeJefatura>1 && jefatura !== '' && jefatura !== null ) {
